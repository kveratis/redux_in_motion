...
class Freezer extends Component {
  ...
  componentDidMount() {
    ...
    setInterval(() => {
      const randomTemperature = -Math.round(Math.random() * 10); // Generate a randome number between 0 & -10
  ...
  handleClickRestock = (flavorName) => {
    const amount = parseInt(window.prompt(`Enter amount to restock ${flavorName}`), 10);
